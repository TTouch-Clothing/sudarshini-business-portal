...
export default function VipCustomersPage() {
  ...
  useEffect(() => {
    http.get('/dashboard/vip-customers').then((r) => setRows(r.data));
  }, []);

  const totalVip = rows?.length || 0;
  const totalPages = Math.max(1, Math.ceil(totalVip / PAGE_SIZE));
  ...
  return (
    <Card title={`VIP Customers (${totalVip})`}>
